	...
	// (Also see getError() below)
{
	...
	if (TSL2561_WriteByte(TSL2561_REG_INTCTL,((control | 3) << 4) & (persist | 15)))
	{
		return true;
	}
	...
	// (Also see getError() below)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c4, (uint16_t) TSL2561_ADDR, &data, 1, 1000) == HAL_OK)
	{
		return true;
	}
...
bool TSL2561_WriteByte(uint8_t regAdr, uint8_t value)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c4, (uint16_t) TSL2561_ADDR, data, 2, 1000) == HAL_OK)
	{
		return true;
	}
	else
...
bool TSL2561_ReadByte(uint8_t regAdr, uint8_t *value)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c4, (uint16_t) TSL2561_ADDR, &data, 1, 1000) == HAL_OK)
	{
		if(HAL_I2C_Master_Receive(&hi2c4, (uint16_t) TSL2561_ADDR, value, 1, 1000) == HAL_OK)
		{
			return true;
		}
		else
	...
	// (Also see getError() above)
{
	...
	if(HAL_I2C_Master_Transmit(&hi2c4, (uint16_t) TSL2561_ADDR, &data, 1, 1000) == HAL_OK)
	{
		if(HAL_I2C_Master_Receive(&hi2c4, (uint16_t) TSL2561_ADDR, tmpValue, 2, 1000) == HAL_OK)
		{
			low = tmpValue[0];
			high = tmpValue[1];
